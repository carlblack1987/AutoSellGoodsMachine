...
namespace Business.Common
{
    public class AsileHelper
    {
        ...
        private List<VendBoxCodeModel> VendBoxList_Pa = new List<VendBoxCodeModel>();
        ...
        public string DoorStatus = "00";

        public AsileModel Asile_Temp = new AsileModel();
        ...
        /// 当前所选择的货柜 2015-05-19
        /// </summary>
        public int CurrentVendBoxIndex
        {
            get;
            set;
        }
        ...
        public bool LoadAsileInfo_Total(string commonType,string isTenAsileNum)
        {
            ...
            try
            {
                ...
                if (dataSet.Tables.Count > 0)
                {
                    ...
                    for (int i = 0; i < recordCount; i++)
                    {
                        ...
                        strVendBoxCode = dataSet.Tables[0].Rows[i]["VendBoxCode"].ToString();
                        ...
                        strIsTenAsileNum = "0";
                        for (int j = 0; j < VendBoxList.Count; j++)
                        {
                            if (VendBoxList[j].VendBoxCode == strVendBoxCode)
                            {
                                strIsTenAsileNum = VendBoxList[j].IsTenAsileNum;
                                break;
                            }
                        }

                        if (strIsTenAsileNum == "1")
                        {
                            // 托盘上有10个货道
            ...
            finally
            {
                ...
                if (VendBoxList.Count > 0)
                {
                    CurrentVendBoxIndex = VendBoxList[0].VendBoxIndex;
                }
            }
            return result;
        }
        ...
        public bool LoadAsileInfo_VendBox()
        {
            ...
            try
            {
                string strVendBoxCode = VendBoxList[CurrentVendBoxIndex].VendBoxCode;
                for (int i = 0; i < AsileList.Count; i++)
                {
                    if (AsileList[i].VendBoxCode == strVendBoxCode)
                    {
    ...
    McdPicName,McdName,SurNum,PaStackNum,SellPrice,McdCode,PaCode) 
        ...
        public bool LoadVendBox()
        {
            ...
            try
            {
                ...
                strSql += @"select VendBoxCode,SellGoodsType,TmpControlModel,TargetTmp,
                    ...
                    TmpRunModel,TargetTmp,TmpBeginTime1,TmpEndTime1,TmpBeginTime2,TmpEndTime2,IsTenAsileNum  
                    from T_VM_VENDBOX";
                        ...
                        for (int j = 0; j < VendBoxList_Pa.Count; j++)
                        {
                            if (VendBoxList_Pa[j].VendBoxCode == dataSet.Tables[0].Rows[i]["VendBoxCode"].ToString())
                            {
                                VendBoxList.Add(new VendBoxCodeModel()
                                {
                                    ...
                                    IsTenAsileNum = dataSet.Tables[0].Rows[i]["IsTenAsileNum"].ToString(),
        ...
        public bool QueryIsOneBox()
        {
            bool result = false;
            for(int i = 0;i<VendBoxList.Count;i++)
            {
                if (VendBoxList[i].VendBoxCode == "1")
                {
                    result = true;
                    break;
                }
        ...
        private BusinessEnum.TmpControlModel ConvertTmpControlModel(string tmpControlModel)
        {
            ...
            switch (tmpControlModel)
            {
                ...
                case "1":// 加热
                    ...
                    break;
                case "2":// 普通
                    eNumTmpControlModel = BusinessEnum.TmpControlModel.Normal;
                    break;
            }
