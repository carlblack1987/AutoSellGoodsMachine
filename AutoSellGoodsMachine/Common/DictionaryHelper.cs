...
namespace AutoSellGoodsMachine
{
    ...
    {
        public static string Dictionary_ChoiceGoods_Main(BusinessEnum.Main_Lgs_TopType lgsType)
        {
            string strChoiceGoods = string.Empty;

            switch (lgsType)
            {
                case BusinessEnum.Main_Lgs_TopType.Normal:
                    strChoiceGoods = PubHelper.p_LangOper.GetStringBundle("Main_ChoiceGoods");
                    break;
                case BusinessEnum.Main_Lgs_TopType.ShangHai_ZhiYuanZhe:
                    strChoiceGoods = PubHelper.p_LangOper.GetStringBundle("Main_ChoiceGoods_Shzy");
                    break;
            }

            return strChoiceGoods;
        }
        ...
        public static string Dictionary_PostCardCode(int postCardCode)
        {
            ...
            switch (postCardCode)
            {
                ...
                case 12:// 余额不足
                    ...
                    break;
                case 14:// 已达限购，无法消费
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("PosCard_Err_OutMoney");
                    break;
                case 15:// 余额超限，无法消费
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("PosCard_Err_OutBanFee");
                    break;
                case 17:// 卡有效期已过
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("PosCard_Err_OutTime");
                    break;
        ...
        /// <summary>
        /// 根据在线会员卡刷卡扣款结果代码获取提示语
        /// </summary>
        /// <param name="postCardCode">刷卡扣款结果代码</param>
        /// <returns>提示语</returns>
        public static string Dictionary_OnlineCardCode(int cardCode)
        {
            string strCodeName = string.Empty;
            ...
            switch (cardCode)
            {
                ...
                case 8:// 调用Web Service失败
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_Net");
                    break;

                case 10:// 不存在会员卡信息
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_NoCard");
                    break;

                case 11:// 黑名单会员
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_BlackCard");
                    break;
                ...
                case 15:// 会员未激活
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_InvalidCard");
                    break;

                case 12:// 余额不足
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_NoEnoughFee");
                    break;
                case 17:// 该卡不能再售货机上消费
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_LimitVm");
                    break;
                case 21:// 会员卡密码错误
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_Pwd");
                    break;

                default:// 认为刷卡失败
                    strCodeName = PubHelper.p_LangOper.GetStringBundle("OnlineCard_Err_Pay");
                    break;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static string Dictionary_Door(int vendBoxIndex,string doorStatus,bool isOneVendBox)
        {
            string strDoorStatus = string.Empty;

            if (isOneVendBox)
            {
                strDoorStatus = doorStatus;
            }
            else
            {
                strDoorStatus = PubHelper.p_BusinOper.AsileOper.VendBoxList[vendBoxIndex].DoorStatus;
            }
            switch (strDoorStatus)
            {
                case "00":// 门关
        ...
        /// </summary>
        /// <returns></returns>
        public static string Dictionary_OnlineEntityCardStatus()
        {
            string strStatus = string.Empty;

            if (PubHelper.p_BusinOper.PaymentOper.PaymentList.Online_EntityCard.ControlSwitch == Business.Enum.BusinessEnum.ControlSwitch.Run)
            {
                switch (PubHelper.p_BusinOper.DeviceInfo.OnlineEntityCardStatus.Status)
                {
                    case "00":
        ...
        public static string Dictionary_QRCodeStatus()
        {
            ...
            if (PubHelper.p_BusinOper.PaymentOper.PaymentList.KimmaOnLineCard.ControlSwitch == Business.Enum.BusinessEnum.ControlSwitch.Run)
            {
        ...
        public static string Dictionary_VendBoxName(string vendBoxCode)
        {
            ...
            else
            {
                ...
                strVendBoxName = strVendBoxTitle + (Convert.ToInt32(vendBoxCode)).ToString();
            }
