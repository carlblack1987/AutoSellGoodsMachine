...
namespace AutoSellGoodsMachine
{
    public class PubHelper
    {
        ...
        public const int MAXTRAYNUM = 12;// 最大托盘数量 + 1
        public const int FONTSIZE_BOX = 22;// 文字大小—货柜
        public const int FONTSIZE_TRAY = 20;// 文字大小—托盘
        ...
        public static string GetPayMentMsgTitle()
        {
            ...
            if (p_BusinOper.PaymentOper.PaymentList.PayShow_IDCode == BusinessEnum.PayShowStatus.Pause)
            {
                ...
                if (intEnablePayNum == 1)
                {
                    ...
                    // 在线会员实体卡
                    if (p_BusinOper.PaymentOper.PaymentList.PayShow_Online_EntityCard == BusinessEnum.PayShowStatus.Normal)
                    {
                        strPayTitle = GetPayMentTitle_OnlineEntityCard();
                        return strPayTitle;
                    }
                    // 支付宝付款码
                    ...
                    // 在线会员卡
                    if (p_BusinOper.PaymentOper.PaymentList.PayShow_KimmaOnLineCard == BusinessEnum.PayShowStatus.Normal)
                    {
                        strPayTitle = GetPayMentTitle_KimmaOnLineCard();
                        return strPayTitle;
                    }
                    // 微信支付
                ...
                else
                {
                    ...
                    {
                        // 在线会员实体卡可用
                        strPayList += PubHelper.p_LangOper.GetStringBundle("Pub_Payment_OnlineEntityCard") + strDunHao;
                    }
                    if (p_BusinOper.PaymentOper.PaymentList.PayShow_UnionPay == BusinessEnum.PayShowStatus.Normal)
                    {
                        // 银联可用
                    ...
                    {
                        // 在线会员虚拟卡可用
                        strPayList += PubHelper.p_LangOper.GetStringBundle("Pub_Payment_KimmaOnlineCard") + strDunHao;
                    }
        ...
        /// <summary>
        /// 根据当前可以支持的支付方式获取需要显示的支付方式提示文本—在线会员实体卡
        /// </summary>
        /// <returns></returns>
        private static string GetPayMentTitle_OnlineEntityCard()
        {
            string strPayTitle = string.Empty;
            switch (p_BusinOper.PaymentOper.PaymentList.PayShow_Online_EntityCard)
            {
                case BusinessEnum.PayShowStatus.Pause:// 不能非储值卡支付
                    strPayTitle = "Err_PauseServer_Reason_OnlineEntityCard";// 不能储值卡支付
                    break;

                case BusinessEnum.PayShowStatus.Normal:// 可以刷卡
                    strPayTitle = "Payment_OnlineEntityCard";// 请刷卡购物
                    break;
        ...
        /// <summary>
        /// 根据当前可以支持的支付方式获取需要显示的支付方式提示文本—在线会员卡支付
        /// </summary>
        /// <returns></returns>
        private static string GetPayMentTitle_KimmaOnLineCard()
        {
            string strPayTitle = string.Empty;

            switch (p_BusinOper.PaymentOper.PaymentList.PayShow_KimmaOnLineCard)
            {
                ...
                case BusinessEnum.PayShowStatus.Normal:// 可以
                    strPayTitle = "Payment_KimmaOnlineCard";// 请刷卡购物
                    break;
        ...
        /// 本地导入商品信息
        /// </summary>
        /// <param name="impType">1：从终端软件目录下导入信息【自动】 2：从移动盘目录下导入信息【手工】</param>
        /// <returns></returns>
        public static int ImportMcdInfo(string impType)
        {
            int intErrCode = 0;

            try
            {
                #region 遍历各盘符，查询是否有信息导入文件

                string strGoodsFilaPath = string.Empty;
                string strGoodsFileName = "goodsimport.txt";
                string strPicSourcePath = string.Empty;
                bool blnFileExist = false;

                if (impType == "1")
                {
                    #region 查询软件目录Import\McdInfo下有没有要导出的文件

                    strGoodsFilaPath = AppDomain.CurrentDomain.BaseDirectory.ToString() + "Import\\McdInfo\\" + strGoodsFileName;
                    if (File.Exists(strGoodsFilaPath))
                    {
                        strPicSourcePath = AppDomain.CurrentDomain.BaseDirectory.ToString() + "Import\\McdInfo\\";
                        // 商品导入文件存在                    
                        blnFileExist = true;
                    }
                    #endregion
                ...
                if (impType == "2")
                {
                   #region 遍历各盘符，查询是否有信息导入文件
                    ...
                    for (int i = 0; i < hexDiskLen; i++)
                    {
                        ...
                        strPicSourcePath = strUDiskFilePath;
                ...
                // 0：成功 1：没有找到信息导入文件 2：读取信息导入文件失败 3：导入失败
                ...
                if (blnFileExist)
                {
                    ...
                    intErrCode = PubHelper.p_BusinOper.GoodsOper.ImportGoodsList(strGoodsListInfo, "0",
                        strPicSourcePath, out updateMcdList);
                    if (intErrCode != 0)
                    {
        ...
        /// <param name="impType">1：从终端软件目录下导入信息【自动】 2：从移动盘目录下导入信息【手工】</param>
        /// <returns></returns>
        public static int ImportAdvInfo(string impType)
        {
            int intErrCode = 0;
            ...
            try
            {
                ...
                if (impType == "1")
                {
                    #region 查询软件目录Import\AdvInfo下有没有要导出的文件

                    strFilaPath = AppDomain.CurrentDomain.BaseDirectory.ToString() + "Import\\AdvInfo\\" + strImportFileName;
                    if (File.Exists(strFilaPath))
                    {
                        strSourcePath = AppDomain.CurrentDomain.BaseDirectory.ToString() + "Import\\AdvInfo\\";
                        // 导入文件存在                    
                        blnFileExist = true;
                    }
                    #endregion
                ...
                if (impType == "2")
                {
                    ...
                    {
                        strUDiskFilePath = hexDiskPath[i] + ":\\";
                        strSourcePath = strUDiskFilePath;
                ...
                if (blnFileExist)
                {
                    ...
                    string updateMcdList = string.Empty;
                    string strAdvListID = string.Empty;// 广告节目单编号
                    intErrCode = PubHelper.p_BusinOper.VmFileOper.ImportAdvertList(strListInfo, "0",
                        strSourcePath,out strAdvListID, out updateMcdList);
                    if (intErrCode != 0)
                    {
                        intErrCode = intErrCode + 2;
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // 广告文件存储正式文件夹
                            string strAdvertDicPath_Formal = AppDomain.CurrentDomain.BaseDirectory.ToString() + "Advert\\" + strAdvListID + "\\";

                            #region 如果广告临时文件夹不存在，则创建

                            bool result = PubHelper.CreatDire(strAdvertDicPath_Temp);
                            result = PubHelper.CreatDire(strAdvertDicPath_Formal);
                            ...
                            for (int i = 0; i < intCount; i++)
                            {
                                ...
                                strImportFilePath = strSourcePath + strAdvertFileNameType;
                                // 检测该广告文件是否存在
                            ...
                            {
                                p_BusinOper.SysCfgOper.UpdateSysCfg("UpdateAdvertListID", p_BusinOper.VmFileOper.UpdateAdvertListID);
                                #region 拷贝广告文件到临时文件夹
        ...
        public static bool CheckIsManager1Purview(string menuId)
        {
            bool result = false;
            if (p_BusinOper.UserType == Business.Enum.BusinessEnum.UserType.SystemUser)
            {
                return true;
            }
            string strPurviewList = p_BusinOper.ConfigInfo.Manager1Purview;
            int i = strPurviewList.IndexOf(menuId + ",");
            if (i >= 0)
            {
                result = true;
            }
            if (!result)
            {
                PubHelper.ShowMsgInfo(p_LangOper.GetStringBundle("Err_NoLimit"), MsgType.Ok);
            }
            return result;
        }

        public static bool CheckIsManager1Purview_NoMsg(string menuId)
        {
            bool result = false;
            if (p_BusinOper.UserType == Business.Enum.BusinessEnum.UserType.SystemUser)
            {
